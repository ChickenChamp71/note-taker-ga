    ...
    if (title && text) {
        ...
        fs.readFile(`app/db/db.json`, 'utf8', (err, data) => {
            if (err) {
                ...
                fs.writeFile(`app/db/db.json`, JSON.stringify(parsedNotes, null, 3), (writeErr) => {
                    if (writeErr) {
        ...
        if (err) {
            ...
            for (let i = 0; i < parseData.length; i++) {
                ...
                if (parseData[i].id == noteId) {
                    ...
                    fs.writeFile(`app/db/db.json`, JSON.stringify(parseData, null, 3), (writeErr) => {
                        if (writeErr) {
                            console.error(writeErr);
